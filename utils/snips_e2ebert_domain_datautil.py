...
def batch_variable(batch_data, Vocab, mode='train'):
    ...
    # label_len = torch.zeros([bsz, 1], dtype=torch.long)
    # for i in range(bsz):
    #     label_len[i] = len(domain2slot[batch_data[i].domain])
    ...
    for i in range(bsz):
        ...
        # tokens_list.append(pref_chars + tokens + ['[PAD]'] * (label_max_len - len(domain2slot[batch_data[i].domain])))
        pref_chars = [slot2desp[x] for x in domain2slot[batch_data[i].domain]]
