...
class BERTVocab(object):
    ...
    def build_vocab(self):
        if self.bert_tokenizer is None and self.bert_model_path is not None:
            ...
            #self.bert_tokenizer = RobertaTokenizer.from_pretrained(self.bert_model_path)
            # self.bert_tokenizer = ElectraTokenizer.from_pretrained(self.bert_model_path)
    ...
    def batch_bertwd2id(self, tokens_lst: list):
        ...
        # size(bsz, max_seq_len, max_seq_len)
        # attn_masks = torch.BoolTensor(bert_masks).unsqueeze(-2).expand(len(tokens_lst), max_bpe_len, max_bpe_len).contiguous()
        # slot_piece = map(transform, [tokenizer.cls_token] + slot_lists[0])
        # slot_piece_list = []
        # for piece in slot_piece:
        #     slot_piece_list.extend(piece)
        # max_slot_bpe_len = len(slot_piece_list)
        # # [CLS] token
        # type_mask_idx = torch.arange(max_slot_bpe_len, dtype=torch.int64)
        # # type_i->type_j = False, type_i->type_i = True
        # # [CLS] can attend to all tokens.
        # attn_masks[:, type_mask_idx, max_slot_bpe_len:] = torch.zeros((len(tokens_lst), max_slot_bpe_len, max_bpe_len-max_slot_bpe_len), dtype=torch.bool).contiguous()
        ...
        #     slot_piece_list.extend(piece)
        # max_slot_bpe_len = len(slot_piece_list)
        # slot_attn_masks = torch.zeros((max_slot_bpe_len, max_slot_bpe_len), dtype=torch.bool).contiguous()
        # slot_num = 0
        # for slot in [tokenizer.cls_token] + slot_lists[0]:
        #     slot_bpe_len = len(transform(slot))
        #     slot_attn_masks[slot_num: slot_num+slot_bpe_len, slot_num: slot_num+slot_bpe_len] = torch.ones((slot_bpe_len, slot_bpe_len), dtype=torch.bool)
        #     slot_num += slot_bpe_len
        # type_mask_idx = torch.arange(max_slot_bpe_len, dtype=torch.int64)
        # attn_masks[:, type_mask_idx.unsqueeze(1), type_mask_idx.unsqueeze(0)] = slot_attn_masks.unsqueeze(0).expand(len(tokens_lst), max_slot_bpe_len, max_slot_bpe_len).contiguous()
        ...
        #     slot_piece_list.extend(piece)
        # max_slot_bpe_len = len(slot_piece_list)
        # text_mask_idx = torch.arange(max_slot_bpe_len, max_bpe_len, dtype=torch.int64)
        # attn_masks[:, text_mask_idx.unsqueeze(1), 1:max_slot_bpe_len] = False
        
        # position_ids = torch.arange(max_bpe_len, dtype=torch.long)
        # position_ids = position_ids.unsqueeze(0).expand(len(bert_ids), max_bpe_len).contiguous()
        # position_ids -= num_type
        # position_ids[:, :num_type+1] = torch.zeros(len(bert_ids), num_type+1)
