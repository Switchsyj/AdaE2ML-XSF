...
class MultiDomainDataLoader(object):
    ...
    def __iter__(self):
        ...
        for dm in dms:
            if len(self.dataset[dm]) > 0:
                ...
                # batch(Instance1, Instance2, ...)
                for idx in idxs:
                    batch.append(self.dataset[dm][idx])
                    if len(batch) == self.batch_size:
                        if self.collate_fn:
                            yield self.collate_fn(batch)
                        else:
    ...
    # def __iter__(self):
    #     dms = [k for k, v in self.dataset.items() if len(v) != 0]
    #     dataset_idxs = {}
    #     # shuffle all the domains and data
    #     if self.shuffle:
    #         random.shuffle(dms)
    #     # self.dataset = {"A": [inst1, inst2, ...], "B": [...], ...}
    #     for dm in dms:
    #         dataset_idxs[dm] = np.random.permutation(len(self.dataset[dm]))
            
    #     max_iter_step = max([math.ceil(len(self.dataset[dm]) / self.batch_size) for dm in dms])
    #     for i in range(max_iter_step):
    #         for _, dm in enumerate(dms):
    #             st = (i * self.batch_size) % len(dataset_idxs[dm])
    #             ed = ((i+1) * self.batch_size) % len(dataset_idxs[dm])
    #             if ed < st:
    #                 fetch_idxs = np.concatenate([dataset_idxs[dm][st:], dataset_idxs[dm][:ed]])
    #             else:
    #                 fetch_idxs = dataset_idxs[dm][st: ed]
                
    #             batch = [self.dataset[dm][index] for index in fetch_idxs]
    #             if self.collate_fn:
    #                 yield self.collate_fn(batch)
    #             else:
    #                 yield batch
    ...
    def __len__(self):
        # dms = list(self.dataset.keys())
        # max_iter_step = max([math.ceil(len(self.dataset[dm]) / self.batch_size) for dm in dms])
        # return max_iter_step * len(dms)
        return sum([((len(self.dataset[dm]) + self.batch_size - 1) // self.batch_size) for dm in list(self.dataset.keys())])
...
def pool(data, batch_size, key, batch_size_fn=lambda new, count, sofar: count,
    ...
    batches.
    """
    for p in batch(data, batch_size * 500, batch_size_fn):
