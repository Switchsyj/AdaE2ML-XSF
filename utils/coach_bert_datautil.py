...
def batch_variable(batch_data, Vocab, mode='train'):
    ...
    for i in range(bsz):
        tokens, template, binary_tags, slu_tags, domain = batch_data[i].tokens, batch_data[i].template_list, batch_data[i].binary_tags, batch_data[i].slu_tags, batch_data[i].domain
    ...
    token_inputs = bert_vocab.batch_bertwd2id(token_list)
    template_inputs = bert_vocab.batch_bertwd2id(template_list)
