...
def batch_variable(batch_data, Vocab, tgt_dm, mode='train'):
    ...
    label_inputs = bert_vocab.batch_bertwd2id(pref_char_list)
    bert_inputs = {
        "bert_ids": {"label": label_inputs[0], "utterance": token_inputs[0]},
        "segments": {"label": label_inputs[1], "utterance": token_inputs[1]},
        "bert_mask": {"label": label_inputs[2], "utterance": token_inputs[2]},
        "bert_lens": {"label": label_inputs[3], "utterance": token_inputs[3]},
    }
    ...
    def to_device(self, device):
        for prop, val in self.__dict__.items():
            if torch.is_tensor(val):
                ...
                # val_ = {}
                def move_to_device(_iter):
                    if torch.is_tensor(_iter):
                        return _iter.to(device)
                    elif isinstance(_iter, collections.abc.Iterable):
                        for k, v in _iter.items():
                            _iter[k] = move_to_device(v)
                        return _iter
                    else:
                ...
                #     val_[k] = v.to(device)
                setattr(self, prop, move_to_device(val))
