...
def read_insts_json(file_reader):
    for line in file_reader:
        new_inst = {"tokens": [], "slu_tags": [], "domain": ""}
        new_inst["tokens"] = line["utterance"]
        new_inst["slu_tags"] = [x.split('-')[0] + '-' + slot2desp[x.split('-')[1]] if x != 'O' else x for x in line["slu_tags"]]
        new_inst["domain"] = line["domain"]
        yield(Instance(**new_inst))

def load_data():
    all_dataset = {}
    for domain in os.listdir('/cognitive_comp/shiyuanjun/zero-shot-slu/data/ATIS'):
        too_long = 0
        with open(f"/cognitive_comp/shiyuanjun/zero-shot-slu/data/ATIS/{domain}/{domain}.txt", 'r', encoding='utf-8') as fr:
            for inst in read_insts(fr, domain):
                if len(inst.tokens) < 512:
                    if all_dataset.get(f"{inst.domain}", None) == None:
                        all_dataset[f"{inst.domain}"] = []
                    all_dataset[f"{inst.domain}"].append(inst)
                else:
                    too_long += 1
        print(f'{too_long} sentences exceeds 512 tokens')
    
    with jsonlines.open('/cognitive_comp/shiyuanjun/zero-shot-slu/data/constsf/e-comSF.json') as f:
        too_long = 0
        for inst in read_insts_json(f):
            if len(inst.tokens) < 512:
                if all_dataset.get(f"{inst.domain}", None) == None:
                    all_dataset[f"{inst.domain}"] = []
                all_dataset[f"{inst.domain}"].append(inst)
...
def create_vocab(all_data_sets, bert_path):
    ...
    domain_set = list(all_data_sets.keys())
