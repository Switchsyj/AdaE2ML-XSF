    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for i, batcher in enumerate(self.test_loader):
                ...
                batch_bio_pred, batch_slu_pred = self.slu_model.evaluate(batch.bert_inputs, num_type=len(domain2slot[batcher[0].domain]), 
                                                                                    num_unseen_type=len(domain2unseen[self.tgt_dm]), mask=batch.token_mask)
        ...
        with open('data/snips/badcase_br_unseen_revise.txt', 'a+') as f:
...
if __name__ == '__main__':
    ...
    print(f"evaluate: {evaluator.evaluate()}")
    print(f"ttt_evaluate: {evaluator.ttt_evaluate()}")