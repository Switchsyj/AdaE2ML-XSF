...
class BinarySLUTagger(nn.Module):
    def __init__(self, 
        ...
        self.lstm = Lstm(num_layers, emb_dim, hidden_size, dropout, bidirectional, freeze_emb, emb_file, vocab=None, use_bert=True)
...
class SlotNamePredictor(nn.Module):
    ...
    def forward(self, domains, hidden_layers, binary_preditions=None, binary_golds=None, final_golds=None, label_reprs=None):
        """
        Inputs:
        ...
        pred_slotname_list = []
        for i in range(bsz):
            slot_embs_based_domain = label_reprs[i]
...
class SentRepreGenerator(nn.Module):
    def __init__(self, emb_dim, freeze_emb, emb_file, hidden_size, num_layers, dropout, bidirectional):
        super(SentRepreGenerator, self).__init__()
        self.hidden_size = hidden_size * 2 if bidirectional else hidden_size
        
        # LSTM Encoder for template
        self.template_encoder = Lstm(num_layers, emb_dim, hidden_size, dropout, bidirectional, freeze_emb, emb_file, vocab=None, use_bert=True)
