...
from modules.bert_mbt import BertEmbedding
...
class End2endSLUTagger(nn.Module):
    ...
    def evaluate(self, bert_inps, num_type, mask=None, tsne=False):
        '''Decoding Rules:
        以边界标签优先，如果预测为O，则不管类别标签是什么，最终标签是O；
        如果预测的是其他类型，则将边界标签和类别标签进行组合，进而评估最终性能
        '''
        bert_repr = self.bert(**bert_inps)
        label_embedding = bert_repr["label"][:, 1:]
        token_repr = bert_repr["utterance"][:, 1:]
    ...
    def forward(self, bert_inps, num_type, boundary_labels, 
        ...
        # orth_loss = ((dot_prod * (ones - diag))**2).sum() / label_embedding.size(0)
        # loss += orth_loss
