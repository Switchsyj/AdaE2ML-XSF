...
class End2endSLUTagger(nn.Module):
    def __init__(self, vocabs, bert_embed_dim,
        ...
        # TODO: Adapter
        # self.label_adapter = nn.Sequential(nn.Linear(self.bert_embed_dim, 512, bias=False),
        #                                     getattr(nn, 'GELU')(),
        #                                     nn.Linear(512, self.bert_embed_dim, bias=False))
        # TODO: LoRA
    ...
    def evaluate(self, bert_inp, num_type, mask=None, tsne=False):
        ...
        # TODO: LoRA
        label_embedding = (F.dropout(bert_repr[:, 1: num_type+1].detach(), p=self.dropout, training=self.training) @ self.lora_A.transpose(0, 1) @ self.lora_B.transpose(0, 1)) + bert_repr[:, 1: num_type+1].detach()
    ...
    def pretrain(self, bert_inp, boundary_labels, O_tag_idx, mask=None):
        bert_repr = self.bert(*bert_inp)
        
        # exclude [CLS] and type label
        token_repr = bert_repr[:, 1:]
        ...
        ptr_loss = boundary_loss.mean()
        return ptr_loss, {"bio_loss": ptr_loss.item()}
    
    def evaluate_ptr(self, bert_inp, mask=None):
        '''Decoding Rules:
        以边界标签优先，如果预测为O，则不管类别标签是什么，最终标签是O；
        如果预测的是其他类型，则将边界标签和类别标签进行组合，进而评估最终性能
        '''
    ...
    def forward(self, bert_inp, num_type, boundary_labels, 
        ...
        if train == 'utter':
            ...
            type_loss = 0.5 * F.cross_entropy(type_score.transpose(1, 2).contiguous(), type_labels, ignore_index=0, reduction='none')  # (B, L)
